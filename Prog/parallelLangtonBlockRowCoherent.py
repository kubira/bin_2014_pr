...
################################################################################
#  Soubor:    parallelLangtonBlockRowCoherent.py                               #
#  Autor:     Radim KUBIŠ, xkubis03                                            #
#  Vytvořeno: 25. dubna 2014                                                   #
#                                                                              #
#  Projekt do předmětu Biologií inspirované počítače (BIN).                    #
#                                                                              #
#  PARALELNÍ VÝPOČET CA LANGTONOVY SMYČKY (PO SOUVISLÝCH BLOCÍCH ŘÁDKŮ)        #
#                                                                              #
...
if rank == 0:
    ...
    for step in range(0, STEPS_OF_CA):
        ...
        for processorIndex in range(1, numberOfProcessors):
            indexFrom = (processorIndex-1)*int(numberOfRows/(numberOfProcessors-1))
            if processorIndex == (numberOfProcessors-1):
                indexTo = numberOfRows
            else:
                indexTo = processorIndex*int(numberOfRows/(numberOfProcessors-1))
            fieldC = None
            fieldC = comm.recv(fieldC, source=MPI.ANY_SOURCE, tag=processorIndex)
            fieldB[indexFrom:indexTo] = fieldC
    ...
    indexFrom = (rank-1)*int(numberOfRows/(numberOfProcessors-1))
    if rank == (numberOfProcessors-1):
        indexTo = numberOfRows
    else:
        indexTo = rank*int(numberOfRows/(numberOfProcessors-1))
    
    # print("%d:%d-%d" % (indexFrom, indexTo, (indexTo-indexFrom)))
    ...
    for step in range(0, STEPS_OF_CA):
        ...
        while rowIndex < indexTo:
            ...
            for colIndex in range(0, numberOfCols):
                ...
                fieldY[rowIndex][colIndex] = getNewLangtonLoopValue(fieldX, rowIndex, colIndex, numberOfCols, numberOfRows)
                
            # Posun na další řádek ke zpracování
            rowIndex += 1

        # Odeslání nového stavu v řádcích root procesoru
        comm.send(fieldY[indexFrom:indexTo], dest=0, tag=rank)
